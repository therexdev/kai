...
async function setup(t) {
  ...
  const tariff = { model: "fixture", version: 1, modelHash: P.hash("model"), tokenizerHash: P.hash("tokenizer"), templateHash: P.hash("template"),
  ...
  const config = { dataDir: path.join(dir, "scheduler"), accounts, koinFundedSessions: { target, observer, meter, clock: f.target.clock, accept: ({ output }) => output === "4" } };
  ...
  const proposal = { model: tariff.model, version: 1, maxOutput: 16, amount: "500", perJob: "200", maxJobs: 3, expires: 2 * 86400000 + 30000 };
  ...
  const quote = () => scheduler.koinFundedSessions.ledger.quote("fixture", 1, [{ role: "user", content: "hello" }], 16);
  ...
  return { ...f, dir, accounts, account, token, grant, base, proposal, target, observer, meter, config, review, authorize, quote, request, post,
    observationId: observation.body.observationId, get ledger() { return scheduler.koinFundedSessions.ledger; },
