...
const { BuildStore, fail } = require("./store"),
  ...
  { BuildChain, SignerClient, wasmHash } = require("./chain"),
  ...
  async tick() {
    ...
    const signedAction = async (action, payload) => {
      for (let i = 0; i < 40; i++) {
        ...
        progress("Transaction submitted · waiting for Koinos finality");
        await new Promise((resolve) => setTimeout(resolve, 3000));
      }
      throw fail(
        "Koinos is still confirming this transaction. Use Retry saved publishing request to resume it.",
        409,
      );
    };
    try {
