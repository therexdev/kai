...
class BuildChain {
  ...
  async submitExact(expected, signed, signerAddress, onVerified = () => {}) {
    if (
      !signed ||
    ...
    // Kondor normally optimizes rc_limit, which changes the transaction ID.
    // Verify the approved action and recovered signer, not the unsigned ID.
    try {
      require("./wallet-proof").verify(expected, signed, signed.id, {
        Transaction, utils,
      }, "Kondor");
    } catch {
      throw Object.assign(fail(
        "The signed transaction changed the reviewed account, network or app action. Reconnect the reviewed account and sign again. Mana adjustments are supported; changing the payer is not.",
      ), { code: "TRANSACTION_MISMATCH", retryable: true });
    }
    const tx = {
      id: signed.id,
    ...
    try {
      ...
      if (result?.receipt?.reverted)
        throw Object.assign(fail("Koinos reverted this app action: " + chainErrorDetail(null, result.receipt), 422), { code: "TRANSACTION_REVERTED" });
    } catch (e) {
      if (e.code === "TRANSACTION_REVERTED") throw e;
      // A response can be lost after acceptance. Check the same ID before
      // returning a node error; never replace or re-sign the user's action.
      try {
        if (await this.confirmed(tx.id)) return tx.id;
      } catch (confirmation) {
        if (confirmation.status === 409 && ["finality", "receipt"].includes(confirmation.confirmationPhase))
          return tx.id;
        if (confirmation.status === 422) throw confirmation;
      }
      throw Object.assign(fail("Koinos submission: " + chainErrorDetail(e.message, e.data), 502), {
