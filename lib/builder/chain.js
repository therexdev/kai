...
class BuildChain {
  constructor(config = settings()) {
    ...
    this.provider.call = async (method, params) => {
      ...
      if (data.error)
        throw fail(
          "Koinos: " +
            String(data.error.message || "request rejected").slice(0, 240),
          502,
        );
      return data.result;
    };
  ...
  async confirmed(txId) {
    ...
    for (const block of result.block_items || []) {
      ...
      if (receipt.reverted)
        throw fail(
          "Koinos reverted this transaction. Check the app ownership and mana limit, then prepare a new request.",
          422,
        );
      return true;
    }
