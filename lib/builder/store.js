...
class BuildStore {
  ...
  detail(account, id) {
    ...
    return {
      ...
      jobs: this.db
        .prepare(
          "SELECT id,kind,status,stage,error,error_detail,json_extract(payload,'$.revision') revision,created_at,updated_at,usage FROM jobs WHERE project_id=? AND account_id=? ORDER BY created_at DESC,rowid DESC LIMIT 8",
        )
        .all(id, account),
    };
  }
