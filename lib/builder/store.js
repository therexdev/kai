...
class BuildStore {
  ...
  enqueue(account, id, kind, payload, limits = {}) {
    return this.transaction(() => {
      ...
      const n = this.db
        .prepare(
          "SELECT count(*) n FROM jobs WHERE account_id=? AND created_at>? AND kind=?",
        )
        .get(account, since, kind).n;
      const all = this.db
        .prepare("SELECT count(*) n FROM jobs WHERE created_at>? AND kind=?")
        .get(since, kind).n;
      if (n >= (limits.perAccount || 10) || all >= (limits.global || 100))
        throw fail(
          "The daily build allowance has been reached. Try again tomorrow.",
          429,
        );
      const jid = uid("job"),
        t = Date.now();
