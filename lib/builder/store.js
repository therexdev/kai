...
class BuildStore {
  constructor(stateDir) {
    ...
    this.db
      ...
      CREATE UNIQUE INDEX IF NOT EXISTS one_active_job ON jobs(project_id) WHERE status IN ('queued','running','confirming');
    ...
    if (!this.db.prepare("PRAGMA table_info(jobs)").all().some((c) => c.name === "error_detail"))
      this.db.exec("ALTER TABLE jobs ADD COLUMN error_detail TEXT");
    if (!this.db.prepare("PRAGMA table_info(jobs)").all().some((c) => c.name === "next_check_at"))
      this.db.exec("ALTER TABLE jobs ADD COLUMN next_check_at INTEGER NOT NULL DEFAULT 0");
    this.transaction(() => {
      this.db.exec("DROP INDEX IF EXISTS one_active_job; CREATE UNIQUE INDEX one_active_job ON jobs(project_id) WHERE status IN ('queued','running','confirming')");
      // Recover requests that older workers incorrectly failed after two minutes
      // of normal finality waiting. Never revive rejected or superseded jobs.
      for (const job of this.db.prepare("SELECT rowid,* FROM jobs WHERE status='failed' AND kind IN ('publish','propose')").all()) {
        if (!/^(The transaction is included in a block but has not reached finality\.|The node has not returned the transaction receipt\.)/.test(job.error || "")) continue;
        const txId = job.error.match(/Transaction: (0x1220[a-f0-9]{64})\s*$/i)?.[1];
        if (!txId) continue;
        if (this.db.prepare("SELECT id FROM jobs WHERE project_id=? AND (status IN ('queued','running','confirming') OR (kind=? AND rowid>?))").get(job.project_id, job.kind, job.rowid)) continue;
        const payload = JSON.parse(job.payload), project = this.owned(job.account_id, job.project_id);
        if (payload.confirmationTxId && payload.confirmationTxId !== txId) continue;
        if (job.kind === "publish" && (!payload.revision || project.live_revision >= payload.revision)) continue;
        this.awaitConfirmation({ ...job, payload }, { txId, phase: "finality" }, 0);
      }
    });
  }
  transaction(fn) {
  ...
  detail(account, id) {
    ...
    return {
      ...
      jobs: this.db
        .prepare(
          "SELECT id,kind,status,stage,error,error_detail,json_extract(payload,'$.revision') revision,json_extract(payload,'$.confirmationTxId') txId,next_check_at,created_at,updated_at,usage FROM jobs WHERE project_id=? AND account_id=? ORDER BY created_at DESC,rowid DESC LIMIT 8",
        )
        .all(id, account),
    };
  }
  ...
  claim() {
    return this.transaction(() => {
      const t = Date.now();
      this.db.prepare("UPDATE jobs SET status='confirming',stage='Resuming transaction confirmation automatically',next_check_at=0,lease_until=0,attempts=0 WHERE status='running' AND lease_until<? AND kind IN ('publish','propose') AND json_extract(payload,'$.confirmationTxId') IS NOT NULL").run(t);
      this.db
        .prepare(
      ...
      const job = this.db
        .prepare(
          "SELECT * FROM jobs WHERE status='queued' OR (status='confirming' AND next_check_at<=?) ORDER BY created_at,rowid LIMIT 1",
        )
        .get(t);
      if (!job) return null;
      this.db
  ...
  awaitConfirmation(job, result, delay = 10000) {
    if (!/^0x1220[a-f0-9]{64}$/i.test(result.txId || ""))
      throw fail("The publishing service did not return a valid transaction ID.", 502);
    if (job.payload.confirmationTxId && job.payload.confirmationTxId !== result.txId)
      throw fail("The pending transaction changed. The saved request needs review.", 409);
    const stage = result.phase === "finality"
      ? "Included in a block · waiting for finality. Checking automatically."
      : result.phase === "receipt"
        ? "Waiting for the transaction receipt. Checking automatically."
        : result.phase === "unavailable"
          ? "Confirmation service is temporarily unavailable. Checking again automatically."
          : "Waiting for block inclusion. Checking automatically.";
    job.payload.confirmationTxId = result.txId;
    this.db.prepare("UPDATE jobs SET status='confirming',stage=?,payload=?,error=NULL,error_detail=NULL,lease_until=0,attempts=0,next_check_at=?,updated_at=? WHERE id=? AND account_id=?")
      .run(stage, JSON.stringify(job.payload), Date.now() + delay, Date.now(), job.id, job.account_id);
  }
  usage(job, tokens) {
    this.db
  ...
  finish(job, error, details = null) {
    this.db
      .prepare(
        "UPDATE jobs SET status=?,stage=?,error=?,updated_at=?,lease_until=0,error_detail=? WHERE id=?",
      )
      .run(
