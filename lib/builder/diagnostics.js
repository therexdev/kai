...
function previewDiagnostics(value, revision) {
  ...
  return value.errors
    ...
    .map((e) => ({
      ...
      phase: ["request", "connect", "read", "prepare", "review", "sign", "submit"].includes(e.phase) ? e.phase : "",
      wallet: ["kondor", "koinvault"].includes(e.wallet) ? e.wallet : "",
      code: clean(e.code).replace(/[^A-Z_]/g, "").slice(0, 60),
