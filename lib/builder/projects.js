...
const fs = require("fs"),
  path = require("path"),
  { fail } = require("./store");
const { FRAME_CONTENT_CSP } = require("./frame-policy");
...
Forms support native validation and submit events. Attach a submit handler and call event.preventDefault() synchronously before awaiting wallet operations. Direct form submissions and external requests are blocked. Do not use form.submit(), external form actions, alert/confirm/prompt, or top-level navigation. Use DOM status messages and dialogs. Wire every visible action to an existing element; show loading, success/pending and error states. The workspace provides preview diagnostics through read_preview_diagnostics; inspect them when fixing interactions. Static validation does not test button clicks, form submissions or wallet flows.
...
kai.call returns {txId,pending?} after wallet signing and submission (or {preview:true} for sample actions). Read tx.txId, not a bare string. Report pending and refresh to check results. Do not claim finality or success until reads show the change. Errors retain message, phase and code. Preserve those details instead of inventing a wallet diagnosis. A successful connection does not prove a transaction can be signed on the app's network. Use inspect_app to inspect deployment and live errors, read_live_source to compare published files, and read_app_contract for actual chain data. These read-only tools do not test a browser or sign a transaction.
...
function validate(files) {
  ...
  for (const name of ["index.html", "app.js"]) {
    if (
      /metamask|eth_requestAccounts|eth_sendTransaction|wallet_switchEthereumChain|wallet_addEthereumChain|walletconnect|(?:window|globalThis|self)\s*(?:\.\s*(?:ethereum|web3)|\[\s*["'](?:ethereum|web3)["']\s*\])|\bethers\s*[.(]|\bnew\s+Web3\b/i.test(
        files[name] || "",
      )
    )
      throw fail(
  ...
  return {
    ...
    note: "Static syntax checks only; button clicks, form submissions and wallet flows have not been tested. Inspect preview diagnostics and try the affected interaction before publishing.",
  };
}
function html(files) {
  validate(files);
