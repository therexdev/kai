...
const FRAME_CSP =
  "sandbox allow-scripts; default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; connect-src 'none'; font-src data:; base-uri 'none'; form-action 'none'; frame-ancestors 'self'";
const SHELL_CSP =
  "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'";
function createBuilderRouter({ accounts, stateDir, builder }) {
  ...
  for (const name of ["build.js", "build.css", "host.js", "bridge.js"])
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      if (d.completed)
        return res.json({ ok: true, txId: d.data.transaction.id });
      const txId = await service.chain.submitExact(
        d.data.transaction,
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      await service.chain.confirmed(d.data.transaction.id);
      const { config } = await service.chain.read(
        p.contract_id,
        "get_config",
        {},
