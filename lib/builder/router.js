...
function createBuilderRouter({ accounts, stateDir, builder }) {
  ...
  router.post(
    ...
    wrap((req, res) => {
      ...
      res.status(202).json({
        ok: true,
        job: store.enqueue(req.account.id, p.id, "edit", {
          prompt,
          revision: p.revision,
  ...
  async function submitDraft(d, transaction) {
    if (d.data.signedTxId && transaction?.wallet === "koinvault")
      throw fail("This request already has a signed transaction. Confirm it before switching wallets.", 409);
    if (d.data.vaultTxId) {
      if (
    ...
    try {
      return await service.chain.submitExact(
        d.data.transaction,
        transaction,
        d.data.signerAddress,
        (txId) => {
          // Re-read after signature recovery: two HTTP requests may have
          // entered concurrently, but only one signed ID may be reserved.
          d.data = JSON.parse(store.db.prepare("SELECT data FROM wallet_drafts WHERE id=?").get(d.id).data);
          if (d.data.vaultTxId || (d.data.signedTxId && d.data.signedTxId !== txId))
            throw fail("This request already has a signed transaction. Retry that transaction to check its outcome.", 409);
          d.data.signedTxId = txId;
          store.db.prepare("UPDATE wallet_drafts SET data=? WHERE id=?")
            .run(JSON.stringify(d.data), d.id);
        },
      );
    } catch (e) {
      const saved = JSON.parse(store.db.prepare("SELECT data FROM wallet_drafts WHERE id=?").get(d.id).data);
      if (!saved.signedTxId && !saved.vaultTxId && e.status && e.status < 500) e.retryable = true;
      else delete e.retryable;
      throw e;
    }
  }
  router.post(
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      if (d.completed)
        return res.json({
          ok: true,
          txId: d.data.vaultTxId || d.data.signedTxId || d.data.transaction.id,
        });
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      await service.chain.confirmed(d.data.vaultTxId || d.data.signedTxId || d.data.transaction.id);
      const { config } = await service.chain.read(
        p.contract_id,
        "get_config",
        {},
  ...
  router.post(
    "/build/public/:slug/diagnostics",
    wrap((req, res) => {
      const p = publicProject(req);
      const account = accounts?.accountOf(req);
      // Only the signed-in owner can supply observations to their AI editor.
      if (!account) return res.json({ ok: true, recorded: false });
      store.owned(account.id, p.id);
      limit("diagnostics:" + account.id, 30);
      res.json({ ok: true, recorded: !!store.recordDiagnostics(account.id, p.id, req.body) });
    }),
  );
  router.post(
