...
class AppData {
  constructor({ stateDir }) {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_spend_account ON spend_events(account_id, created_at DESC);
    `);

    /*
     * Zero cost has two meanings — the free daily allowance absorbed it, or
     * the person's own machine answered and there was nothing to bill. Only
     * paidWith tells them apart, so it has to survive a reload; without it a
     * refresh quietly relabels "your own machine" as "free allowance".
     *
     * ADD COLUMN on an existing table, guarded by a read of the schema —
     * SQLite has no ADD COLUMN IF NOT EXISTS, and this runs on every boot.
     */
    const cols = this.db.prepare("PRAGMA table_info(messages)").all().map((c) => c.name);
    if (!cols.includes("paid_with")) {
      this.db.exec("ALTER TABLE messages ADD COLUMN paid_with TEXT");
    }
  }
  ...
  messages(accountId, chatId) {
    this.ownedChat(accountId, chatId);
    return this.db
      .prepare("SELECT id, role, content, created_at, served_model, cost_micro, paid_with FROM messages WHERE chat_id = ? ORDER BY created_at, rowid")
      .all(String(chatId))
      .map((m) => ({
        id: m.id,
        role: m.role,
        ...
        paidWith: m.paid_with || null,
      }));
  }

  addMessage(accountId, chatId, { role, content, servedModel, costMicro, paidWith }) {
    this.ownedChat(accountId, chatId);
    ...
    const mid = id("msg");
    const t = now();
    this.db.prepare("INSERT INTO messages (id, chat_id, role, content, created_at, served_model, cost_micro, paid_with) VALUES (?,?,?,?,?,?,?,?)")
      .run(mid, String(chatId), String(role), String(content ?? "").slice(0, MAX_CONTENT), t,
        ...
        paidWith ? String(paidWith).slice(0, 40) : null);
  ...
  /**
   * CLAIM a due task: move its clock forward, atomically, and report whether
   * this caller was the one that won.
   *
   * This exists because a run can take minutes — the consume path waits up to
   * 180s for a provider — while the runner ticks every 60. setInterval does
   * not wait for an async callback, so tick N+1 would find a task that tick N
   * was still executing, see it as due, and run it AGAIN. On something that
   * spends money with nobody watching, that is a double charge arriving by
   * accident.
   *
   * The guard is the WHERE clause, not a flag in memory: `next_run_at <= ?`
   * is re-evaluated by sqlite inside the UPDATE, so of two callers racing,
   * exactly one sees changes === 1. Claiming BEFORE the work also means a
   * process killed mid-run resumes at the next interval rather than
   * immediately — the conservative direction for a spend loop.
   */
  claimTask(taskId, at = now()) {
    const t = this.db.prepare("SELECT * FROM tasks WHERE id = ?").get(String(taskId));
    if (!t) return false;
    const r = this.db.prepare("UPDATE tasks SET next_run_at = ? WHERE id = ? AND next_run_at <= ? AND enabled = 1")
      .run(at + Number(t.every_minutes) * 60000, String(taskId), at);
    return r.changes === 1;
  }

  /**
   * Record what happened. Does NOT touch the clock — claimTask already moved
   * it, on success and failure alike, which is what keeps a failing task from
   * staying permanently due and being retried every tick.
   *
   * A manual "Run now" therefore leaves the schedule exactly where it was:
   * running something by hand is not a statement about when it should next
   * run by itself.
   */
  recordRun(taskId, { ok, output, error, pause }) {
    ...
    this.db.prepare(
      ...
      ok ? null : String(error || "failed"), pause ? 0 : Number(t.enabled), String(taskId));
  ...
  /* ----------------------------------------------------------------- spend */
  /*
   * A line per charge: when, which surface spent it, which class answered,
   * and how much.
   *
   * The grant already tells you how much is left. It cannot tell you where
   * the rest went — and once TASKS are running unattended, "where did my
   * three dollars go" is a question the product has to be able to answer.
   * Without this, the honest answer is "somewhere", which is not one.
   *
   * Free answers are recorded too, at zero. A day the allowance covered
   * everything should look like a day with usage on it, not a day with none.
   */

  recordSpend(accountId, { grantId, source, label, model, costMicro }) {
    const micro = Math.max(0, Math.round(Number(costMicro) || 0));
    this.db.prepare(
      "INSERT INTO spend_events (id, account_id, grant_id, source, label, model, cost_micro, created_at) VALUES (?,?,?,?,?,?,?,?)"
    ).run(id("spend"), String(accountId), grantId ? String(grantId) : null, String(source),
      String(label || "").slice(0, MAX_TITLE) || null, model ? String(model) : null, micro, now());
    // Bounded: keep the most recent window rather than growing without end.
    // A ledger nobody prunes eventually becomes the reason a page will not
    // load, and the useful part of this is always the recent part.
    this.db.prepare(
      `DELETE FROM spend_events WHERE account_id = ? AND id NOT IN
         (SELECT id FROM spend_events WHERE account_id = ? ORDER BY created_at DESC LIMIT ?)`
    ).run(String(accountId), String(accountId), MAX_SPEND_EVENTS);
  }

  spend(accountId, limit = 50) {
    const rows = this.db
      .prepare("SELECT * FROM spend_events WHERE account_id = ? ORDER BY created_at DESC LIMIT ?")
      .all(String(accountId), Math.min(Number(limit) || 50, MAX_SPEND_EVENTS));
    const totals = this.db
      .prepare("SELECT COUNT(*) AS n, COALESCE(SUM(cost_micro),0) AS micro FROM spend_events WHERE account_id = ?")
    ...
    return {
      events: rows.map((r) => ({
        ...
        label: r.label || null,
        model: r.model || null,
        costUsd: Number(r.cost_micro) / 1e6,
        createdAt: Number(r.created_at),
      })),
      // Totals over the RETAINED window, and the view says so rather than
      // implying it is the lifetime figure — the grant's own spentUsd is that.
      totalUsd: Number(totals.micro) / 1e6,
      count: Number(totals.n),
      retained: MAX_SPEND_EVENTS,
    };
  }
  ...
  purgeAccount(accountId) {
    ...
    /*
     * The spend log goes too. It is a record of what this person did here —
     * which chat, which document — and "delete my data" that quietly keeps a
     * timestamped list of your activity is not a deletion. The grant keeps
     * its own spent total, which is a financial fact about a wallet rather
     * than a record of behaviour, and that stays.
     */
    const sp = this.db.prepare("DELETE FROM spend_events WHERE account_id = ?").run(String(accountId));
    return { chats: r.changes, docs: d.changes, tasks: k.changes, memories: m.changes, spendEvents: sp.changes };
  }
}
