...
class JsonStore {
  ...
  // Bounded cursor walk for recovery, including records older than the UI's
  // latest-200 window. No pruning or rewriting of earned rewards.
  epochSummaryPage(before = Number.MAX_SAFE_INTEGER, limit = 100) {
    let epochs = [];
    try {
      epochs = fs.readdirSync(this.dataDir).map((n) => /^epoch-(\d+)\.json$/.exec(n))
        .filter(Boolean).map((m) => Number(m[1])).filter((n) => n < before)
        .sort((a, b) => b - a).slice(0, limit);
    } catch { /* empty store */ }
    return epochs.map((epoch) => ({ epoch, summary: this.readEpoch(epoch)?.summary || null }));
  }
  listEpochSummaries() {
    const out = [];
    try {
...
class SqliteStore {
  ...
  _migrateFromJson() {
    ...
    // Re-derive fresh JSON views immediately (review finding): the renames
    // just emptied the dir of live *.json ledgers, and the 6-hourly backup
    // bundles only *.json files — without this, every post-cutover snapshot
    // until the first epoch close would be EMPTY.
    try { this.exportViews(); } catch { /* best-effort */ }
  }
  ...
  epochSummaryPage(before = Number.MAX_SAFE_INTEGER, limit = 100) {
    // Extract only the summary, not every historical receipt blob.
    return this.db.prepare("SELECT epoch, json_extract(data, '$.summary') AS summary FROM epochs WHERE epoch < ? ORDER BY epoch DESC LIMIT ?")
      .all(before, limit).map((row) => ({ epoch: Number(row.epoch), summary: row.summary ? JSON.parse(row.summary) : null }));
  }
  listEpochSummaries() {
    // Newest 200 only (review finding): the table grows unbounded — one row
    // per 15-min epoch forever — and loading every receipt blob to serve
    // /operator/epochs or the boot settlement scan would become an event-loop
    // stall. 200 matches the state-backup retention cap.
    const out = [];
    for (const row of this.db.prepare("SELECT data FROM epochs ORDER BY epoch DESC LIMIT 200").all()) {
      try {
    ...
    return out.reverse(); // ascending, like the file-scan order
  }
  ...
  exportViews() {
    ...
    // Newest 200 epochs only (review finding): rewriting the ENTIRE history
    // every 15-min close is O(all-time) sync work that only grows. 200 covers
    // everything the backup/tooling reads; older epochs are anchored on-chain.
    for (const row of this.db.prepare("SELECT epoch, data FROM epochs ORDER BY epoch DESC LIMIT 200").all()) {
...
function openStore(dataDir, mode = process.env.KAI_STORE) {
  if (mode !== "sqlite") return new JsonStore(dataDir);
  try {
    return new SqliteStore(dataDir);
  } catch (e) {
    // A store that can't open must not crash-loop the site (systemd
    // Restart=always would hammer it forever). Fall back to the flat files
    // LOUDLY: `degraded` is surfaced on /api/health so a flip that didn't
    // take is visible from outside within one check. The fallback reads the
    // exported views (same filenames, refreshed every epoch close), so it
    // boots at most one close stale — degraded, not amnesiac.
    console.error(`[store] CRITICAL: sqlite store failed to open (${e.message}) — falling back to json`);
    const js = new JsonStore(dataDir);
    js.degraded = `sqlite-unavailable: ${e.message}`.slice(0, 200);
    return js;
  }
}
