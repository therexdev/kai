...
class RewardClaims {
  ...
  async operation(id) {
    ...
    const args = { epoch: r.epoch, account: enc(utils.decodeBase58(c.address)), availability: c.availability, work: c.work,
      proof: c.proof.map(p => ({ hash: enc(Buffer.from(p.hash, "hex")), availability: p.availability, work: p.work, left: p.left })) };
