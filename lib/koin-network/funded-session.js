...
class FundedSessionObserver {
  ...
  async #capture(id, owner) {
    ...
    if (!c || !off(config.paused) || c.chain_id !== t.chainId || c.credits !== account(t.credits) ||
    ...
    if (!off(s.closed) || number(s.revoked_at) !== 0n || !remaining || !perJob || !maxJobs || maxJobs > 10000n ||
        jobs >= maxJobs || number(s.nonce) !== jobs) throw Error("Session revoked, closed or exhausted");
    if (!opened || opened > BigInt(after.time) || expires <= opened || expires > opened + BigInt(DAY) ||
        settleUntil !== expires + BigInt(DAY) || now < opened || now >= expires || BigInt(after.time) >= expires) throw Error("Session expired or invalid time limits");
    ...
    return { id, owner, head: after, state: { config, session: s, balances },
      stateHash: hash(JSON.stringify({ config, session: s, balances })) };
  }
  async observe({ id, owner }) {
    ...
    const snapshot = await this.#capture(id, owner);
    if (this.#snapshots.size >= 32) throw Error("Observation queue full");
  ...
  async verify(observationId) {
    ...
    const current = await this.#capture(s.id, s.owner);
    ...
    return { ...result, state: "verified", chainId: this.#target.chainId, credits: this.#target.credits,
