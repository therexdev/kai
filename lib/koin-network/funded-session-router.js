"use strict";
// Explicitly configured, authenticated rehearsal. No settlement signer or broadcast capability.
const path = require("path");
const P = require("./job-protocol"), D = require("./session-delegation");
const { FundedReservations } = require("./funded-reservations");
function createFundedSessionRouter({ dataDir, accounts, observer, target, meter, accept = null, settlementPolicy = null, work: workConfig = null, clock = Date.now }) {
  if (!accounts) throw Error("Funded session rehearsal requires accounts");
  let work;
  const ledger = new FundedReservations(path.join(dataDir, "koin-funded-sessions"), { accounts, observer, target, meter, clock, settlementPolicy,
    accept: workConfig ? value => work.accept(value) : accept });
  if (workConfig) work = require("./funded-work").createFundedWork({ ...workConfig, ledger, accounts, meter, target, accept, clock });
  ...
  async function handle(req, res, workerAuth) {
    if (work && await work.handle(req, res, workerAuth)) return true;
  ...
  return { handle, ledger, work, close: () => { work?.close(); ledger.close(); } };
}
