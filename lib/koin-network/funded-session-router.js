...
function createFundedSessionRouter({ dataDir, accounts, observer, target, meter, accept = null, work: workConfig = null, clock = Date.now }) {
  if (!accounts) throw Error("Funded session rehearsal requires accounts");
  let work;
  const ledger = new FundedReservations(path.join(dataDir, "koin-funded-sessions"), { accounts, observer, target, meter, clock,
