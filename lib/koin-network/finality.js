...
async function inspectFinality(provider, { chainId, txId, expectedOperation }) {
  ...
  if (!Array.isArray(response.transactions)) throw Error("Missing transaction lookup result");
  ...
  if (!Array.isArray(record.containing_blocks) || record.containing_blocks.length > 100) throw Error("Invalid containing blocks");
  if (!record.containing_blocks.length) return { state: "pending", txId };
  record.containing_blocks.forEach(id);
  const head = await provider.getHeadInfo(), lib = uint(head.last_irreversible_block);
  id(head.head_topology?.id);
  if (lib > uint(head.head_topology.height)) throw Error("Invalid irreversibility height");
  const candidates = await provider.getBlocksById(record.containing_blocks, { returnBlock: false, returnReceipt: false });
  if (!Array.isArray(candidates.block_items)) throw Error("Missing block lookup result");
  let reversible = false;
  for (const b of candidates.block_items) {
    if (!record.containing_blocks.includes(b.block_id)) throw Error("Unexpected containing block");
