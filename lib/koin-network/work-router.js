...
function createWorkRouter({ dataDir, operatorSecret, domain, meter, qualify, accept, fundingObserver = null, clock = Date.now,
  accounts = null, consumerBindings = null, consumerWaitMs = 180000 }) {
  ...
  const { createGrantBindings, createGrantChat } = require("./grant-chat");
  const bindings = consumerBindings === null ? null : createGrantBindings({ accounts, bindings: consumerBindings, meter });
  const ledger = new ShadowJobs(path.join(dataDir, "koin-shadow-work"), { domain: P.domain(domain), meter, clock,
    authorizeDelegated: bindings?.authorize });
  const payloads = new Map();
  const consumer = bindings ? createGrantChat({ ledger, meter, bindings, payloads, sweep, clock, waitMs: consumerWaitMs }) : null;
  ...
  function sweep() {
    const jobs = ledger.recover();
    for (const j of jobs) if (["reserved", "dispatched"].includes(j.state) && j.delegatedAuthorization && !consumer?.authorizeJob(j)) ledger.cancel(j.id);
    ...
    try {
      if (url.pathname === "/koin/shadow/jobs/result" && req.method === "POST") {
        ...
        if (["verified", "prepared", "submitted", "settled"].includes(j.state)) {
          ...
          consumer?.remember(j, b.output);
          send(res, 200, { ok: true, accepted: true, receiptHash: j.receiptHash }); return true;
        }
        if (j.delegatedAuthorization && !consumer?.authorizeJob(j)) {
          if (["reserved", "dispatched"].includes(j.state)) ledger.cancel(j.id);
          payloads.delete(j.id); throw Error("The spending grant changed or was revoked");
        }
        ...
        consumer?.remember(done, b.output);
      ...
      if (url.pathname === "/koin/shadow/jobs/grant") {
        ...
        send(res, 200, { ok: true, evidence: await fundingObserver.observe({ id: P.digest(b.id), owner: b.owner, purpose: b.purpose }) });
  ...
  return { handle, next, ledger, consumer,
    busy: (address) => ledger.recover().some((j) => j.state === "dispatched" && j.provider === address),
    close: () => { consumer?.close(); payloads.clear(); ledger.close(); } };
}
