...
class ShadowJobs {
  constructor(dataDir, { domain, meter, clock = Date.now } = {}) {
  ...
  reserve({ id, session, quoteHash, signature }) {
    ...
    return this.tx(() => {
      ...
      const j = { id, session, mode: "shadow", state: "reserved", quote: q,
        hold: hold.toString(), reservedAt: at, requestSignature: signature };
  ...
  status(session) {
    return this.tx(() => {
      ...
      return { mode: "shadow", paymentsEnabled: false, spent: g.spent,
