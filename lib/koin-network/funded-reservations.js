...
// Funded accounting rehearsal. No worker dispatch, settlement, signer, network
// routes or broadcasts. All processes for a deployment MUST share this DB.
...
class FundedReservations {
  #db; #observer; #target; #clock; #meter;
  constructor(directory, { observer, target, meter, clock = Date.now }) {
  ...
  #exposure(id) {
    const holds = this.#db.prepare("SELECT data FROM holds WHERE session=? AND state!='cancelled'").all(id);
    return { count: holds.length, held: holds.reduce((n, h) => n + uint(JSON.parse(h.data).amount), 0n) };
  }
  async #evidence(observationId) {
    const e = await this.#observer.verify(P.digest(observationId));
    if (e.state !== "verified" || e.paymentsEnabled !== false || e.spendingAuthorized !== false) throw Error("Irreversible funding evidence required");
    ...
    this.#fresh(e); return e;
  }
  #fresh(e) {
    const now = P.integer(this.#clock());
    if (!Number.isSafeInteger(e.checkedAt) || now < e.checkedAt || now - e.checkedAt > 5000 || BigInt(now) >= uint(e.expires)) throw Error("Stale funding evidence");
  }
  #sync(e) {
    this.#fresh(e);
  ...
  async markDispatched({ id, observationId, provider }) {
    ...
    const result = this.#tx(() => {
      ...
      h.state = "dispatched"; h.provider = provider; h.dispatchedAt = now; this.#saveHold(h);
      return { hold: h };
    });
    if (result.error) throw Error(result.error);
...
module.exports = { FundedReservations, reservationHash };
