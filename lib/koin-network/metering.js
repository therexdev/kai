...
class Meter {
  ...
  constructor(entries = []) {
    for (const { tariff, adapter } of entries) {
      ...
      this.#entries.set(key, Object.freeze({ tariff: Object.freeze(t), input: adapter.input.bind(adapter), output: adapter.output.bind(adapter),
        render: typeof adapter.render === "function" ? adapter.render.bind(adapter) : null,
        encode: typeof adapter.encode === "function" ? adapter.encode.bind(adapter) : null }));
    }
  ...
  tariffs() {
    return [...this.#entries.keys()].sort().map(key => structuredClone(this.#entries.get(key).tariff));
  }
  quote(domain, model, version, input, maxOutput, at) {
    ...
    const prompt = e.render ? e.render(m) : JSON.stringify(m);
    const q = { schema: 2, mode: "shadow", domain, policyHash: this.policyHash, tariff: t,
      requestHash: hash(JSON.stringify(m)), promptHash: hash(prompt), inputIdsHash: e.encode ? hash(JSON.stringify(e.encode(prompt))) : null,
