...
/**
 * Control panels and .env files often add quotes or stray whitespace around a
 * pasted value. Strip those so an otherwise-correct hash still works.
 */
...
/**
 * Explain whether a stored hash is usable. A malformed value can never match
 * any password, so the caller can report that instead of failing every login.
 */
function describeHash(raw) {
  ...
  if (parts[0] !== "scrypt") {
    return { ok: false, reason: `does not start with "scrypt$" (got "${parts[0].slice(0, 12)}…")` };
  }
  if (parts.length !== 6) {
    return {
      ok: false,
      reason:
        `expected 6 "$"-separated segments, found ${parts.length}. ` +
        "The $ signs were likely eaten by a shell or .env parser, or the value was truncated",
    };
  }
  const [, N, r, p, salt, key] = parts;
  if (!Number(N) || !Number(r) || !Number(p)) {
    return { ok: false, reason: "cost parameters are not numbers" };
  }
  if (Buffer.from(salt, "base64").length < 8 || Buffer.from(key, "base64").length < 16) {
    return { ok: false, reason: "salt or key is too short — the value looks truncated" };
  }
  return { ok: true, reason: "", params: { N: Number(N), r: Number(r), p: Number(p) } };
}

function verifyPassword(password, stored) {
  try {
    const parts = normalizeHash(stored).split("$");
