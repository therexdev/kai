"use strict";
const { complete: payoutComplete, RETRY_MS: PAYOUT_RETRY_MS } = require("./payouts");
...
// How long a worker's registration proof stays valid. Same 120s as a consume
// signature and for the same reason: long enough that a machine with a lazy
// NTP client still registers, short enough that a captured proof is worthless
// before anyone can use it — and it is single-use inside the window anyway.
const REGISTER_SIG_WINDOW_MS = 120000;
// Producer-only rows (#84): bounded, and stale ones stop being shown. A card
// claiming a machine is producing when it went quiet hours ago is worse than
// no card, which is the same rule the registration path already applies.
const PRODUCER_MAX = Number(process.env.KAI_PRODUCER_MAX || 5000);
const PRODUCER_TTL_MS = Number(process.env.KAI_PRODUCER_TTL_MS || 30 * 60 * 1000);
...
class Scheduler {
  constructor({ dataDir, operatorSecret, koinShadow, koinWork, koinFundedSessions, chain, settlement, epoch, leaseMs, priceSources, bootstrapPoolSat, freeTokensPerDay, freeTokensPerDayGlobal, freeTokensPerIp, royalties, splits, jobModel, preferWindowMs, classEnforce, clampEnforce, reputationEnforce, workerProofEnforce, storeMode, accounts, onEvent } = {}) {
    ...
    this._settlementInFlight = new Map();
    this._settlementTail = Promise.resolve();
    this._settlementRecovery = null;
    this._settlementCursor = Number.MAX_SAFE_INTEGER;
    this._payoutStatus = { version: 1, running: false, lastRunAt: null, lastEpoch: null, lastStatus: null };
    ...
    // Desktop-wallet requests are charged after generation. Serialize that
    // lane per wallet so concurrent requests cannot all approve against the
    // same pre-charge capacity. Browser grants reserve atomically instead.
    this._consumerAddresses = new Set();
    ...
    // Explicitly enabled shadow telemetry; no financial signer or settlement route.
    const shadowConfig = koinShadow ?? (process.env.KAI_KOIN_SHADOW === "1" ? { audience: process.env.KAI_KOIN_AUDIENCE } : null);
    this.koinShadow = shadowConfig ? require("./koin-network/shadow-router").createShadowRouter({
      dataDir: path.join(this.dataDir, "koin-shadow"), operatorSecret: this.operatorSecret, ...shadowConfig,
    }) : null;
    this.onEvent = onEvent || (() => {});
    this.koinWork = koinWork ? require("./koin-network/work-router").createWorkRouter({
      ...koinWork, accounts, dataDir: this.dataDir, operatorSecret: this.operatorSecret,
    }) : null;
    this.koinFundedSessions = koinFundedSessions ? require("./koin-network/funded-session-router").createFundedSessionRouter({
      ...koinFundedSessions, accounts, dataDir: this.dataDir,
    }) : null;
    this._rehearsalPolls = new Set();
    this.workers = new Map(); // token -> {address, capabilities, lastSeen}
    /*
     * Block producers that are NOT selling AI compute (#84).
     *
     * A SEPARATE map, deliberately, and this is the whole safety argument.
     * The obvious design is a `producerOnly` flag on a normal worker row, but
     * `this.workers` is read in a dozen places — routing candidates, the
     * payout roster, workersOnline, reputation, queue hygiene, the auto-model
     * chooser — most of them filtering `[...this.workers.values()]` inline
     * rather than through _liveWorkers(). A flag would have to be remembered
     * at every one of those sites, and at every site added later; miss one and
     * a machine that sells nothing starts receiving jobs it cannot serve, or
     * drawing a share of a payout it did not earn.
     *
     * Keeping these rows out of `this.workers` entirely makes that class of
     * mistake unrepresentable. Every existing filter excludes them because it
     * cannot see them, and so does every future one, without anyone having to
     * know this feature exists.
     *
     * In memory only, on purpose. It is a display card; clients re-report on a
     * timer, so a restart costs a blank until the next report. That is exactly
     * the trade the registration path already makes ("a stale card claiming
     * otherwise is worse than a brief blank"), and it guarantees a
     * producer-only row can never be loaded back in as a worker.
     */
    this.producers = new Map(); // address -> {address, producer, firstSeen, lastSeen}
    ...
    /*
     * FIND-NET-001. /worker/register took the address on trust: anyone who
     * could reach the scheduler could claim to be any wallet, receive that
     * wallet's jobs and land on the payout roster under it. The fix is a
     * signature over the address, and the ONLY reason this is a flag rather
     * than an unconditional check is the rollout: every node already earning
     * runs a client that does not sign yet, and refusing them would empty the
     * network the moment this deploys.
     *
     * So SHADOW first — proofs are verified and recorded on every worker,
     * unsigned registrations are accepted and marked — and KAI_WORKER_PROOF_ENFORCE=1
     * closes the door once /scheduler/network/status?detail shows the roster
     * fully signed. A BAD proof is refused in both modes: shadow tolerates
     * clients too old to sign, never a client that signed wrong.
     */
    this.workerProofEnforce = workerProofEnforce ?? process.env.KAI_WORKER_PROOF_ENFORCE === "1";
  ...
  /**
   * The block-producer card for an address, wherever it was reported from.
   *
   * A machine that BOTH earns and produces reports via /worker/register, so
   * the worker row wins and nothing changes for it. A machine that only
   * produces reports via /producer/report and is found in the second map.
   * Callers ask this one question and do not need to know which.
   *
   * Stale producer-only rows are not returned: the client re-reports on a
   * timer, so silence means the node stopped, and a card still quoting a VHP
   * figure from hours ago would be a confident lie. Worker rows are left to
   * their own liveness rules, which the account page already reflects.
   */
  producerFor(address, now = Date.now()) {
    for (const w of this.workers.values()) {
      if (w.address === address) return w.producer || null;
    }
    const p = this.producers.get(String(address));
    if (!p) return null;
    if (now - p.lastSeen > PRODUCER_TTL_MS) return null;
    return p.producer || null;
  }
  ...
  statsPublic({ detail = false } = {}) {
    ...
    const out = {
      ...
      /*
       * FIND-NET-001 rollout counter, public and undetailed on purpose: the
       * health digest reads this endpoint every check, and "how many nodes
       * still cannot prove the wallet they are being paid as" is the one
       * number that decides when KAI_WORKER_PROOF_ENFORCE gets armed. It
       * reaching 0 and staying there is the go signal.
       */
      workersUnsigned: live.filter((w) => (w.proof || "unsigned") !== "signed").length,
      workerProofEnforced: !!this.workerProofEnforce,
    ...
    if (detail) {
      ...
      out.workers = live.map((w) => ({
        ...
        // FIND-NET-001 rollout: "signed" once this node proved the address it
        // claims. The roster reading all-signed is the go signal for
        // KAI_WORKER_PROOF_ENFORCE=1.
        proof: w.proof || "unsigned",
      }));
  ...
  /**
   * Record a consume signature and report whether it had already been used.
   * Prunes anything past the freshness window on the way through, so the map
   * cannot outgrow one window of requests.
   */
  /* Single-use guard, shared by every signed client protocol (consume
     requests and worker registration proofs). Keyed by the signature bytes,
     so two domains can never collide: their signed messages differ by an
     unambiguous prefix, and different messages cannot share a signature. */
  _signatureReplayed(signature, ts) {
    if (!this._usedConsumeSigs) this._usedConsumeSigs = new Map();
    const now = Date.now();
    if (this._usedConsumeSigs.size > 64) {
      for (const [k, expiry] of this._usedConsumeSigs) if (expiry <= now) this._usedConsumeSigs.delete(k);
    }
    const id = crypto.createHash("sha256").update(String(signature || "")).digest("hex");
    const seen = this._usedConsumeSigs.get(id);
    if (seen && seen > now) return true;
    // Keep it exactly as long as the ts window would still accept it.
    const until = (Number.isFinite(ts) ? ts : now) + CONSUME_SIG_WINDOW_MS;
    this._usedConsumeSigs.set(id, Math.max(until, now + 1000));
    return false;
  }
  ...
  async _handle(req, res) {
    if (this.koinFundedSessions && await this.koinFundedSessions.handle(req, res, r => this._auth(r))) return;
    if (this.koinWork && await this.koinWork.handle(req, res, (r) => this._auth(r))) return;
    if (this.koinShadow && await this.koinShadow.handle(req, res)) return;
    ...
    /*
     * A block producer that sells no AI compute (#84).
     *
     * Before this, the producer snapshot could only ride along with
     * /worker/register, and the client only registers inside earn.start() — so
     * a machine running a Koinos node with Earning switched off never reported
     * at all, and its owner's account page showed no producer card. v0.46.2
     * made that visible ("the report needs Earning on") without fixing it.
     *
     * This route creates NO worker: the row lands in this.producers, which
     * routing, payouts and workersOnline cannot see. It hands back no token,
     * because a token is the right to be dispatched to and that is precisely
     * what must not be granted here.
     *
     * The signature is REQUIRED, with no shadow mode. /worker/register
     * tolerates unsigned clients because thousands of older installs predate
     * the check and cutting them off would stop them earning. Nothing has ever
     * called this route, so it has no such debt, and an unauthenticated
     * version would let anyone publish invented VHP figures onto somebody
     * else's account page. New surface, no legacy, so the bar starts high.
     *
     * Domain-separated from both siblings: "producer|" cannot be confused with
     * "register|" or "consume|", so no captured signature from either can be
     * replayed here, nor one from here used to claim a worker slot.
     */
    if (url.pathname === "/producer/report" && req.method === "POST") {
      const b = await this._body(req);
      if (!b.address) return this._json(res, 400, { ok: false, error: "address required" });

      const ts = Number(b.ts);
      if (!Number.isFinite(ts) || Math.abs(Date.now() - ts) > REGISTER_SIG_WINDOW_MS) {
        return this._json(res, 401, { ok: false, error: "stale producer proof — check this machine's clock" });
      }
      if (this._signatureReplayed(String(b.signature), ts)) {
        return this._json(res, 401, { ok: false, error: "that producer proof has already been used — sign a new one" });
      }
      const hash = crypto.createHash("sha256").update(`producer|${b.address}|${ts}`).digest();
      let signer;
      try {
        signer = Signer.recoverAddress(hash, Buffer.from(String(b.signature), "base64"));
      } catch {
        return this._json(res, 401, { ok: false, error: "bad producer proof" });
      }
      if (signer !== b.address) {
        return this._json(res, 401, { ok: false, error: "producer proof does not match the address being claimed" });
      }

      const snapshot = sanitizeProducer(b.producer);
      if (!snapshot) {
        // Nothing worth showing: forget the address rather than keep a husk
        // that renders as an empty card forever.
        this.producers.delete(String(b.address));
        return this._json(res, 200, { ok: true, stored: false });
      }

      // Bounded, and oldest-first eviction. This map is written by anyone who
      // can sign for an address, so it must not be a way to grow the process
      // without limit.
      const prev = this.producers.get(String(b.address));
      if (!prev && this.producers.size >= PRODUCER_MAX) {
        let oldestKey = null;
        let oldestAt = Infinity;
        for (const [k, v] of this.producers) {
          if (v.lastSeen < oldestAt) { oldestAt = v.lastSeen; oldestKey = k; }
        }
        if (oldestKey) this.producers.delete(oldestKey);
      }
      this.producers.set(String(b.address), {
        address: String(b.address),
        producer: snapshot,
        firstSeen: prev?.firstSeen || Date.now(),
        lastSeen: Date.now(),
      });
      return this._json(res, 200, { ok: true, stored: true });
    }
    ...
    if (url.pathname === "/worker/register" && req.method === "POST") {
      ...
      /*
       * FIND-NET-001: prove the address belongs to whoever is claiming it.
       *
       * The proof is a wallet signature over sha256("register|<address>|<ts>").
       * The "register|" prefix is load-bearing: a consume request signs
       * "consume|<address>|<ts>|<messages>" with the same key, and without
       * distinct domains a captured consume signature — which a paying client
       * hands to this very server on every network request — would be a valid
       * registration proof for that wallet.
       *
       * Three outcomes, and the middle one is the whole rollout:
       *   signed   — verified; the address is proven.
       *   unsigned — no ts/signature at all. A client too old to know about
       *              this. Accepted in shadow, refused under enforce.
       *   bad      — a ts/signature was sent and did not check out. REFUSED in
       *              both modes. Tolerating old clients is a schedule; letting
       *              a wrong signature through would make the check theatre.
       */
      let proof = "unsigned";
      if (b.signature != null || b.ts != null) {
        proof = "bad";
        const ts = Number(b.ts);
        if (!Number.isFinite(ts) || Math.abs(Date.now() - ts) > REGISTER_SIG_WINDOW_MS) {
          return this._json(res, 401, { ok: false, error: "stale registration proof — check this machine's clock" });
        }
        // Single-use inside the window: a proof captured off the wire (or out
        // of a log) cannot be replayed to claim the address a second time.
        if (this._signatureReplayed(String(b.signature), ts)) {
          return this._json(res, 401, { ok: false, error: "that registration proof has already been used — sign a new one" });
        }
        const hash = crypto.createHash("sha256").update(`register|${b.address}|${ts}`).digest();
        let signer;
        try {
          signer = Signer.recoverAddress(hash, Buffer.from(String(b.signature), "base64"));
        } catch {
          return this._json(res, 401, { ok: false, error: "bad registration proof" });
        }
        if (signer !== b.address) {
          return this._json(res, 401, { ok: false, error: "registration proof does not match the address being claimed" });
        }
        proof = "signed";
      } else if (this.workerProofEnforce) {
        return this._json(res, 401, {
          ok: false,
          error: "this scheduler requires a signed registration — update Koinos AI to a version that proves wallet ownership",
        });
      }
      ...
      // A desktop has one runtime slot even when it has several model files.
      // New clients name the resident class their automatic probes should use
      // so fairness checks do not force a multi-GB model swap every few
      // minutes. It is only a hint and only valid when it is also in the
      // server-filtered model list; paid requests can still use every class.
      const preferredModel = models.includes(String(b.capabilities?.preferredModel || ""))
        ? String(b.capabilities.preferredModel)
        : null;
      ...
      this.workers.set(token, {
        address: b.address,
        capabilities: { ...(b.capabilities || {}), preferredModel },
        models,
        ...
        /*
         * FIND-NET-001 rollout signal. Deliberately NOT carried over from
         * prev, unlike fingerprint: this records what THIS registration
         * proved, and a machine that downgrades to an unsigned client must
         * show up as unsigned again. Inheriting it would let one signed
         * registration launder every later one.
         */
        proof,
        /*
      ...
      return this._json(res, 200, { ok: true, token, epoch: this.epoch, koinShadow: !!this.koinShadow });
    }
          ...
          out.push({
            ...
            settlement: sum.settlement
              ? {
                  ...
                  status: sum.settlement.status ?? null,
                  nextRetryAt: sum.settlement.nextRetryAt ?? null,
                }
              : null,
          });
        }
    ...
    if (url.pathname === "/worker/next-job" && req.method === "GET") {
      ...
      if (!w) return this._json(res, 401, { ok: false, error: "bad token" });
      w.lastSeen = Date.now();
      const lockPoll = !!(this.koinFundedSessions?.work || this.koinWork);
      if (lockPoll && this._rehearsalPolls.has(w.address)) return this._json(res, 204, { ok: true, job: null });
      if (lockPoll) this._rehearsalPolls.add(w.address);
      try {
        const funded = this.koinFundedSessions?.work;
        if (funded?.busy(w.address) || this.koinWork?.busy(w.address)) return this._json(res, 204, { ok: true, job: null });
        if (funded && !this._busySet().has(w.address)) {
          try {
            const job = await funded.next(w, () => !res.destroyed && !this._busySet().has(w.address));
            if (job) return this._json(res, 200, { ok: true, job });
          } catch { return this._json(res, 204, { ok: true, job: null }); }
        }
        // Explicitly opted-in shadow work uses its own durable ledger and never
        // enters the legacy queue, receipt list, reputation or billing paths.
        if (this.koinWork && !this._busySet().has(w.address)) {
          if (this.koinWork.busy(w.address)) return this._json(res, 204, { ok: true, job: null });
          const shadow = this.koinWork.next(w);
          if (shadow) return this._json(res, 200, { ok: true, job: shadow });
        }
        const give = () => {
        ...
        return;
      } finally { if (lockPoll) this._rehearsalPolls.delete(w.address); }
    }
    ...
    if (url.pathname === "/worker/result" && req.method === "POST") {
      ...
      /*
       * The result must come from the worker the job was LEASED to
       * (FIND-NET-002). Everything below proves the sender is a registered
       * worker and that the signature is really theirs — but never that this
       * job was theirs, so any registered worker could answer any other
       * worker's in-flight job: taking the receipt and the credit for work it
       * did not do, and deciding what the consumer received.
       *
       * Bound on ADDRESS, not token: re-registration mints a new token for the
       * same machine (standby resume, watchdog, token refresh) and those must
       * still be able to hand back work already in flight. A worker whose
       * lease expired and was re-dispatched elsewhere is correctly refused
       * here — the job belongs to whoever holds it now.
       */
      if (job.worker && job.worker !== w.address) {
        return this._json(res, 409, {
          ok: false,
          error: "that job is leased to a different worker",
        });
      }
    ...
    if (url.pathname === "/consume/chat/completions" && req.method === "POST") {
      const b = await this._body(req);
      if (b?.billing !== undefined) {
        if (b.billing === "koin-funded-rehearsal" && this.koinFundedSessions?.work) return this.koinFundedSessions.work.chat(req, res, b);
        if (b.billing !== "koin-shadow" || !this.koinWork?.consumer) return this._json(res, 409,
          { error: { message: "KOIN rehearsal is not enabled on this scheduler", type: "koin_shadow_error" }, paymentsEnabled: false });
        return this.koinWork.consumer.handle(req, res, b);
      }
      ...
      const consumeTs = Number(b.ts);
      if (!grantCharge && (!Number.isFinite(consumeTs) || Math.abs(Date.now() - consumeTs) > CONSUME_SIG_WINDOW_MS)) {
        return this._json(res, 401, { error: { message: "stale or invalid request signature timestamp — check this machine's clock", type: "invalid_request_error" } });
      }
      if (!grantCharge) {
      ...
      /*
       * Single-use inside its freshness window (FIND-SEC-001).
       *
       * The signed message covers the address, the timestamp and the messages
       * — so the same bytes replayed were a valid, billable request, over and
       * over, charged to the wallet that signed it once. It also does not
       * cover the model or max_tokens, which means a captured request could be
       * replayed against a pricier class than the one its owner agreed to;
       * refusing the replay outright is what takes that away today. Binding
       * the billable terms into the signature itself is the client-side other
       * half and lands with the worker-identity protocol change.
       *
       * Bounded by construction: nothing older than the window can be
       * presented at all, so the table only ever holds one window's traffic.
       */
        if (this._signatureReplayed(String(b.signature), consumeTs)) {
          return this._json(res, 401, {
            error: { message: "that signed request has already been used — sign a new one", type: "invalid_request_error" },
          });
        }
        if (this._consumerAddresses.has(b.address)) {
          return this._json(res, 409, {
            error: { message: "an earlier request for this wallet is still running", type: "invalid_request_error" },
          });
        }
      ...
      const result = await new Promise((resolve) => {
        ...
        const finish = (value) => {
          ...
          if (value === null) {
            // A disconnected caller cannot consume the answer or authorize
            // its charge. Remove both queued and leased forms so abandoned
            // work cannot later turn into a receipt.
        ...
        timer = setTimeout(disconnected, 180000); // big classes stream for minutes; chunks keep it live
        timer.unref?.();
      ...
      return this._json(res, 200, {
        ...
        if (walletLocked) this._consumerAddresses.delete(b.address);
      }
    }
  ...
  settleClosedEpoch(summary) {
    if (!this.settlement || !summary || !summary.receipts || this._closed) return Promise.resolve(null);
    ...
    if (this._settlementInFlight.has(summary.epoch)) return this._settlementInFlight.get(summary.epoch);
    const work = this._settlementTail.then(async () => {
      if (this._closed) return null;
      // Re-read after taking the lock: operator retries and the timer must not
      // overwrite newer progress using a stale copy of a summary.
      const stored = this.store.readEpoch(summary.epoch);
      if (!stored?.summary || stored.summary.root !== summary.root) {
        throw new Error("Payout summary is not durably stored or its root changed");
      }
      const current = stored.summary;
      this._payoutStatus.running = true;
      this._payoutStatus.lastEpoch = current.epoch;
      const checkpoint = async (result) => {
        const record = this.store.readEpoch(current.epoch);
        if (!record?.summary || record.summary.root !== current.root) throw new Error("Payout checkpoint root mismatch");
        current.settlement = JSON.parse(JSON.stringify(result));
        record.summary = current;
        // STRICT: never submit when the pre-broadcast checkpoint can't persist.
        this.store.saveEpoch(current.epoch, record);
        summary.settlement = current.settlement;
      };
      try {
        let result;
        try {
          result = await this.settlement.settleEpoch(current, checkpoint);
        } catch (e) {
          result = { ...current.settlement, status: "pending", error: String(e.message).slice(0, 200),
            settledAt: new Date().toISOString(), nextRetryAt: new Date(Date.now() + PAYOUT_RETRY_MS).toISOString() };
        }
        await checkpoint(result);
        this._balanceCache.clear();
        this._payoutStatus.lastStatus = result.status || (payoutComplete(current) ? "complete" : "pending");
        this.onEvent({ type: payoutComplete(current) ? "scheduler:epoch-settled" : "scheduler:settle-pending", epoch: current.epoch, rootTx: result.rootTx });
        return result;
      } finally {
        this._payoutStatus.running = false;
        this._payoutStatus.lastRunAt = new Date().toISOString();
      }
    });
    this._settlementTail = work.catch(() => {});
    this._settlementInFlight.set(summary.epoch, work);
    work.finally(() => this._settlementInFlight.delete(summary.epoch)).catch(() => {});
    return work;
  }

  // One bounded pass, on boot AND periodically. Walk all stored epochs with a
  // cursor so older partial failures cannot fall outside a newest-N cutoff.
  recoverPendingSettlements(maxEpochs = 2) {
    if (!this.settlement || this._closed) return Promise.resolve(0);
    if (this._settlementRecovery) return this._settlementRecovery;
    const run = async () => {
      let retried = 0;
      const started = Date.now();
      const page = this.store.epochSummaryPage(this._settlementCursor, 100);
      for (const row of page) {
        if (this._closed) break;
        this._settlementCursor = row.epoch;
        const s = row.summary;
        if (!s?.receipts || s.persisted === false || payoutComplete(s) || s.settlement?.status === "blocked") continue;
        if (Date.parse(s.settlement?.nextRetryAt) > Date.now()) continue;
        await this.settleClosedEpoch(s);
        retried++;
        if (retried >= Math.max(1, Math.min(10, maxEpochs)) || Date.now() - started >= 120000) break;
      }
      if (!page.length || this._settlementCursor === page.at(-1)?.epoch && page.length < 100) {
        this._settlementCursor = Number.MAX_SAFE_INTEGER;
      }
      return retried;
    };
    const work = run();
    this._settlementRecovery = work;
    work.finally(() => { this._settlementRecovery = null; }).catch(() => {});
    return work;
  }

  payoutStatus() {
    return { enabled: !!this.settlement, automaticRecovery: !!this._payoutTimer, ...this._payoutStatus };
  }

  startPayoutRecovery(intervalMs = PAYOUT_RETRY_MS) {
    if (this._payoutTimer || !this.settlement || this._closed) return;
    const recover = () => this.recoverPendingSettlements().catch((e) => {
      this.onEvent({ type: "scheduler:recovery-failed", message: String(e.message).slice(0, 200) });
    });
    this._payoutTimer = setInterval(recover, intervalMs);
    this._payoutTimer.unref?.();
    recover();
  }
  ...
  close() {
    this._closed = true;
    clearInterval(this._payoutTimer);
    ...
    return new Promise((resolve) => {
      const done = () => {
        this._settlementTail.finally(() => {
          try { this.store.close(); } catch { /* already closed */ }
          try { this.koinWork?.close(); } catch { /* already closed */ }
          try { this.koinFundedSessions?.close(); } catch { /* already closed */ }
          resolve();
        });
      };
...
function seedMysteryOnce(sched) {
  ...
  const preferred = target.capabilities?.preferredModel;
  const model = target.models.includes(preferred)
    ? preferred
    : target.models[Math.floor(Math.random() * target.models.length)];
...
function seedOnce(sched) {
  ...
  if (target) {
    const preferred = target.capabilities?.preferredModel;
    if (target.models.includes(preferred)) {
      model = preferred;
    } else {
      // Older clients have no resident-model hint; preserve their existing
      // fair rotation rather than silently changing protocol behaviour.
...
function startAutoOps(sched, { seedMs = 45000, epochMs = 15 * 60 * 1000 } = {}) {
  sched.startPayoutRecovery(); // also required when mounted by the production website
