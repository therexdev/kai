 ...
 * (mirrored from therexdev/kaiapp server/scheduler.js — keep in sync).
...
const MODEL_RATES = {
  ...
  // §52 per-class sims).
  "koinos-fast": { minRamGb: 4, inMicroPerM: 100000, outMicroPerM: 400000, royaltyBps: 0, ctxTokens: 4096 }, // $0.10 / $0.40 per 1M
...
class Scheduler {
  constructor({ dataDir, operatorSecret, chain, settlement, epoch, leaseMs, priceSources, evalCapPerEpoch, bootstrapCapSat, freeTokensPerIp, royalties, splits, jobModel, preferWindowMs, onEvent } = {}) {
    ...
    this._workersPath = path.join(dataDir, "workers.json");
    try {
      for (const [token, w] of Object.entries(JSON.parse(fs.readFileSync(this._workersPath, "utf8")))) {
    ...
    this.perf = {}; // address -> {jobs, tokPerSec, cuRating, srvTokPerSec, ok, to, bad, sr}
  ...
  _persistWorkers() {
    try {
      fs.mkdirSync(this.dataDir, { recursive: true });
      fs.writeFileSync(this._workersPath, JSON.stringify(Object.fromEntries(this.workers)));
    } catch {
      /* best-effort */
    }
  }
  ...
  _busySet(now = Date.now()) {
    return new Set(
      [...this.pending.values()]
        .filter((p) => now - Math.max(p.dispatchedAt ?? now, p.lastActivityAt || 0) < (p.leaseMs || this.leaseMs))
        .map((p) => p.worker)
    );
  }
  ...
  statsPublic({ detail = false } = {}) {
    ...
    const live = [...this.workers.values()].filter((x) => now - x.lastSeen < 90000 || busy.has(x.address));
    const models = {};
    for (const w of live) {
    ...
    const out = {
      ...
      models: Object.entries(models)
        ...
        .map(([model, providers]) => ({ model, providers })),
  ...
  _reapPending() {
    ...
    for (const [id, job] of this.pending) {
      ...
      if (now - startedAt <= JOB_ABS_CAP_MS && now - activeAt < (job.leaseMs || this.leaseMs)) continue;
      this.pending.delete(id);
      const { worker, dispatchedAt, dispatchId, leaseMs, preferWorker, preferUntil, lastActivityAt, ...fresh } = job;
    ...
    for (let i = this.queue.length - 1; i >= 0; i--) {
      ...
      if (j.forWorker && !liveWorkers.some((w) => w.address === j.forWorker)) {
        this._unseed(j.forWorker);
        delete j.forWorker;
        changed = true;
      }
      // Protocol evals NO live worker can serve are garbage after a grace
      ...
      // those and seeding halts network-wide (review finding). Consumer
      // jobs are never dropped here; their failures surface to consumers.
      if (j.type === "inference-eval" && now - Date.parse(j.createdAt) > 5 * 60000) {
        if (!liveWorkers.some((w) => this._canServe(w, j))) {
          if (j.forWorker) this._unseed(j.forWorker);
          this.queue.splice(i, 1);
          this.onEvent({ type: "scheduler:eval-dropped", jobId: j.id, model: j.model });
          changed = true;
        }
      }
    }
  ...
  async handle(req, res) {
    ...
    if (url.pathname === "/worker/result" && req.method === "POST") {
      ...
      // can see. Typical tokenizers run ~4 chars/token; the cap allows 2
      // chars/token before clamping, so no honest report ever hits it —
      // only inflation does. Clamps are logged, not (yet) punished.
      const promptChars = (job.messages ? JSON.stringify(job.messages) : String(job.prompt || "")).length;
      const outChars = String(b.output ?? "").length;
      const promptCapTok = 16 + Math.ceil(promptChars / 2);
      const outCapTok = 16 + Math.ceil(outChars / 2);
      const reported = {
      ...
      if (usage.prompt_tokens < reported.prompt_tokens || usage.completion_tokens < reported.completion_tokens) {
        this.onEvent({ type: "scheduler:usage-clamped", worker: w.address, jobId: b.jobId, reported, clamped: usage });
      }
      ...
      if (honest && usage.completion_tokens > 0 && job.dispatchedAt) {
        ...
        const stps = +(usage.completion_tokens / secs).toFixed(2);
        const p = this.perf[w.address];
    ...
    if (url.pathname === "/consume/chat/completions" && req.method === "POST") {
      ...
      const job = this.enqueue({ type: "chat", cu: 1, messages: b.messages, model });
      ...
      const { paidWith, costMicro, freeTaken, totalTok } = this._chargeUsage(b.address, result.usage, clientIp, model);
  ...
  closeEpoch() {
    ...
    // seed to each new joiner for hours until it "caught up").
    for (const w of this.workers.values()) w.seedsThisEpoch = 0;
    const served = {};
    const byWorker = {};
 ...
 *  strings). Tiered by the class's size: every task must be comfortably
 ...
 *  success rate). Expected answers never leave the scheduler. Failure
 *  rates per class are a field-watch item before tightening further. */
function makeChallenge(model) {
  const ri = (n) => Math.floor(Math.random() * n);
  const kinds = [
    () => {
      const a = 2 + ri(30), b = 2 + ri(30);
      return { prompt: `What is ${a}+${b}? Reply with just the number.`, expected: String(a + b) };
    },
    () => {
      const pairs = [
      ...
      return { prompt: `Name the capital of ${country} in one word.`, expected: capital };
    },
  ];
  // Bigger classes (≥12GB) get slightly harder tasks a toy substitute
  // trips on; still trivial for anything the class legitimately holds.
  if ((MODEL_RATES[model]?.minRamGb || 4) >= 12) {
    kinds.push(() => {
    ...
    kinds.push(() => {
      ...
      return { prompt: `Write the word "${w}" in all capital letters, no spaces.`, expected: w.toUpperCase() };
    });
  }
...
function startAutoOps(sched, { seedMs = 45000, epochMs = 15 * 60 * 1000 } = {}) {
  const seed = setInterval(() => seedOnce(sched), seedMs);
...
module.exports = { Scheduler, merkleRoot, merkleProof, startAutoOps, seedOnce, makeChallenge, MODEL_RATES };
