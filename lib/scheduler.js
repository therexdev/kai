...
class Scheduler {
  ...
  _reapPending() {
    ...
    for (const [id, job] of this.pending) {
      ...
      const { worker, dispatchedAt, dispatchId, leaseMs, preferWorker, preferUntil, ...fresh } = job;
...
function seedOnce(sched) {
  ...
  const busy = new Set(
    [...sched.pending.values()].filter((p) => now - p.dispatchedAt < (p.leaseMs || PENDING_LEASE_MS)).map((p) => p.worker)
  );
...
module.exports = { Scheduler, merkleRoot, merkleProof, startAutoOps, seedOnce, MODEL_RATES };
