...
class Scheduler {
  constructor({ dataDir, operatorSecret, chain, settlement, epoch, leaseMs, priceSources, bootstrapPoolSat, freeTokensPerDay, freeTokensPerDayGlobal, freeTokensPerIp, royalties, splits, jobModel, preferWindowMs, classEnforce, clampEnforce, onEvent } = {}) {
    ...
    this._perfPath = path.join(this.dataDir, "perf.json");
    this.perf = {}; // address -> {jobs, tokPerSec, cuRating, srvTokPerSec, ok, to, bad, sr, chal, clamps, clampEgregious}
    try {
      const saved = JSON.parse(fs.readFileSync(this._perfPath, "utf8"));
      if (saved && typeof saved === "object" && !Array.isArray(saved)) this.perf = saved;
    } catch {
      /* fresh — no persisted perf yet */
    }
    // 'exit' handlers run synchronously even through process.exit() — which is
    // how the runtime-log's SIGTERM/crash handlers terminate — so perf earned
    // mid-epoch survives every shutdown path, not just a clean epoch close.
    // (_persistPerf is fully sync: writeFileSync + renameSync.) Deregistered
    // in close() so test suites constructing many schedulers don't leak
    // listeners.
    this._onExit = () => this._persistPerf();
  ...
  /** Atomic file write (tmp + rename): a crash/SIGTERM mid-write can truncate
   *  the tmp file but never the target, so the last good ledger always
   *  survives a host recycle. Every money-path/persistence write goes through
   *  here (durable-storage phase 0 — the flat-JSON hardening that precedes the
   *  SQLite cutover, docs/durable-ledger-design.md). */
  ...
  _saveBalances() {
    try {
      this._atomicWrite(this._creditsPath, JSON.stringify(this.balances, null, 2));
    } catch {
      /* best-effort */
    }
  }
  ...
  _persistWorkers() {
    try {
      this._atomicWrite(this._workersPath, JSON.stringify(Object.fromEntries(this.workers)));
    } catch {
      /* best-effort */
    }
  }
  ...
  _persistPerf() {
    try {
      this._atomicWrite(this._perfPath, JSON.stringify(this.perf));
    } catch {
  ...
  async handle(req, res) {
    ...
    if (url.pathname === "/operator/revoke" && req.method === "POST") {
      ...
      this._atomicWrite(this._revokedPath, JSON.stringify(this.revoked, null, 2));
  ...
  _networkSubsidyBudget(receipts = this.receipts) {
    let demandSat = 0n;
    for (const r of receipts) if (r.honest) demandSat += this._subsidyValueSat(r);
    return { poolSat: this.bootstrapPoolSat, demandSat };
  }
   ...
   * Splits (§20) divide only what actually minted. NOTE: this bounds TOTAL
  ...
  _settleFor(receipts, budget = this._networkSubsidyBudget()) {
    ...
    const { poolSat, demandSat } = budget;
    ...
    for (const r of receipts) {
      ...
      const allowedSat = minted(subsidySat);
  ...
  closeEpoch() {
    ...
    // Persist rolling perf every epoch so a restart resumes routing quality +
    // reputation signals instead of cold-starting them.
    this._persistPerf();
  ...
  async settleClosedEpoch(summary) {
    ...
    try {
      ...
      j.summary = summary;
      this._atomicWrite(file, JSON.stringify(j, null, 2));
    } catch {
      /* best-effort */
    }
    return result;
