...
class Scheduler {
  constructor({ dataDir, operatorSecret, chain, settlement, epoch, leaseMs, priceSources, bootstrapPoolSat, freeTokensPerDay, freeTokensPerDayGlobal, freeTokensPerIp, royalties, splits, jobModel, preferWindowMs, classEnforce, clampEnforce, onEvent } = {}) {
    ...
    this._freeDayPath = path.join(dataDir, "freeday.json");
    try {
      const fd = JSON.parse(fs.readFileSync(this._freeDayPath, "utf8"));
    ...
    if (epoch == null) {
      try {
        ...
        if (latest != null) {
          const j = JSON.parse(fs.readFileSync(path.join(dataDir, `epoch-${latest}.json`), "utf8"));
  ...
  _saveFreeDay() {
    try {
      this._atomicWrite(
        this._freeDayPath,
        JSON.stringify({ day: this.freeDay, byAddr: this.freeUsedByDay, global: this.freeUsedGlobalDay, byIp: this.freeUsedByIp })
      );
    } catch {
      /* best-effort */
    }
