...
class Scheduler {
  constructor({ dataDir, operatorSecret, chain, settlement, epoch, leaseMs, priceSources, bootstrapPoolSat, freeTokensPerDay, freeTokensPerDayGlobal, freeTokensPerIp, royalties, splits, jobModel, preferWindowMs, classEnforce, clampEnforce, reputationEnforce, storeMode, onEvent } = {}) {
  ...
  closeEpoch() {
    ...
    // the summary written but the counters unbumped (or vice versa). In json
    // mode it degrades to the same sequential atomic writes as before.
    try {
      ...
      /* best-effort — an epoch close must never crash the process */
    }
  ...
  _persist(summary) {
    try {
      this.store.saveEpoch(this.epoch, {
        epoch: this.epoch,
