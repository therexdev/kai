...
class Scheduler {
  ...
  _reputation(address, now = Date.now(), worker = null) {
    ...
    const ageDays = w && w.firstSeen ? Math.max(0, (now - numOr(w.firstSeen, now)) / 86400000) : 0;
  ...
  closeEpoch() {
    ...
    for (const r of this.receipts) {
      ...
      if (r.jobType === "chat") chatByAddr[r.worker] = (chatByAddr[r.worker] || 0) + 1;
    }
    // Accrue the durable paid-demand counter the shadow reputation reads, and
    // persist once (not per-receipt) so a reboot keeps the history. A pure
    // Sybil can't manufacture real consumer chats, so this is a Sybil-hard
    // signal that strengthens as real usage grows.
