...
/*
 * Koinos AI scheduler — M2 alpha, mounted into the koinosai.io website
 * (mirrored from therexdev/kaiapp server/scheduler.js — keep in sync).
 * Project-operated; workers
...
class Scheduler {
  constructor({ dataDir, operatorSecret, chain, onEvent } = {}) {
    ...
    // Unix-minutes epoch numbers: unique + monotonic across restarts, so an
    // on-chain submit_root can never collide with a previous run's epoch.
    this.epoch = Math.floor(Date.now() / 60000);
    this.server = null;
  }
 ...
 *  workers always have work; ~20% carry hidden known-answer challenges. */
const SEED_PROMPTS = [
...
function startAutoOps(sched, { seedMs = 45000, epochMs = 15 * 60 * 1000 } = {}) {
  const seed = setInterval(() => {
    const activeWorkers = [...sched.workers.values()].filter((w) => Date.now() - w.lastSeen < 90000);
    if (activeWorkers.length === 0) return;
    if (sched.queue.length + sched.pending.size >= 3) return;
    const job = SEED_PROMPTS[Math.floor(Math.random() * SEED_PROMPTS.length)];
    sched.enqueue(job);
  }, seedMs);
