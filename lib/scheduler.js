...
class Scheduler {
   ...
   *  survive a restart. Written once per epoch close (not per job — the perf
   *  object mutates on every result). Atomic tmp+rename so a crash mid-write
   *  can't corrupt the last good perf. */
  _persistPerf() {
    try {
      fs.mkdirSync(this.dataDir, { recursive: true });
      const tmp = this._perfPath + ".tmp";
      fs.writeFileSync(tmp, JSON.stringify(this.perf));
      fs.renameSync(tmp, this._perfPath);
    } catch {
