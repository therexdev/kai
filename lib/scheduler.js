  ...
  async handle(req, res) {
    ...
    if (url.pathname === "/consume/chat/completions" && req.method === "POST") {
      ...
      if (!b.signature && (b.sessionToken || b.grantId)) {
        ...
        const acct = this.accounts.sessionAccount(String(b.sessionToken || ""));
        if (!acct) {
      ...
      if (grantCharge) {
        ...
        if (worstMicro > grantCharge.remainingMicro) {
      ...
      /*
       * Book the ACTUAL cost against the grant. Never the worst case — the
       * bound above exists to refuse impossible requests, not to charge for
       * tokens nobody generated. A throw here would mean the cap was hit
       * between the check and now (a concurrent request); the work is already
       * done and the ledger already charged, so record it and let the NEXT
       * request be refused rather than losing the receipt over it.
       */
      if (grantCharge && costMicro > 0) {
        try {
          this.accounts.chargeGrant(grantCharge.grantId, costMicro);
        } catch {
          this.onEvent({ type: "grant:cap-exceeded-in-flight", grant: grantCharge.grantId, costMicro });
        }
      }
