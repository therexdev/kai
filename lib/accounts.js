...
class AccountService {
  constructor({ stateDir, sendMail = null, siteOrigin = "https://koinosai.com", google = null, onEvent = () => {} }) {
    ...
    this.db.exec(`
      ...
      /* One row per grant authorization ever accepted, keyed by the digest of
         the signed message. A re-sent authorization has the same digest, so
         the INSERT below is what makes a grant signature single-use
         (FIND-FIN-002). */
      CREATE TABLE IF NOT EXISTS grant_nonces (
        digest TEXT PRIMARY KEY, address TEXT NOT NULL, used_at INTEGER NOT NULL);
      CREATE INDEX IF NOT EXISTS idx_grant_nonces_used ON grant_nonces(used_at);
  ...
  /**
   * Account row by id, with no credential involved.
   *
   * Nothing reachable from the network may call this — it answers "does this
   * account exist" without asking "are you it". Its one caller is the web
   * app's task runner, which has already established the account from a task
   * row it wrote itself and has no session to present because there is no
   * browser. Everything that money depends on is still checked downstream
   * against the grant.
   */
  accountById(id) {
    return this.db.prepare("SELECT * FROM accounts WHERE id = ?").get(String(id || "")) || null;
  }
  ...
  /** Every live session, so a person can SEE what is signed in as them.
   *  Never the token or its hash — only enough to recognise and revoke. */
  sessionList(account, currentToken) {
    const curHash = currentToken ? sha256hex(currentToken) : null;
    return this.db
      .prepare("SELECT token_hash, label, created_at, last_used_at, expires_at FROM sessions WHERE account_id = ? AND expires_at > ? ORDER BY last_used_at DESC")
      .all(account.id, now())
      .map((r) => ({
        // A short, non-reversible handle: enough to name one session in a
        // revoke call, useless for authenticating as it.
        id: r.token_hash.slice(0, 12),
        label: r.label || "unknown device",
        createdAt: Number(r.created_at),
        lastUsedAt: Number(r.last_used_at),
        expiresAt: Number(r.expires_at),
        current: curHash ? r.token_hash === curHash : false,
      }));
  }

  /** End one session by its short handle. */
  revokeSessionById(account, id) {
    const h = String(id || "");
    if (!/^[0-9a-f]{12}$/.test(h)) throw Object.assign(new Error("not a session id"), { status: 400 });
    const row = this.db
      .prepare("SELECT token_hash FROM sessions WHERE account_id = ? AND substr(token_hash,1,12) = ?")
      .get(account.id, h);
    if (!row) throw Object.assign(new Error("no such session"), { status: 404 });
    this.db.prepare("DELETE FROM sessions WHERE token_hash = ?").run(row.token_hash);
  }

  /*
   * Sign out everywhere. The recovery action for "I think someone has my
   * account" — which had no answer at all before, on a system where a session
   * can now spend money. Grants are revoked alongside, because a stolen
   * session whose grant survives is still a live drain: ending the session
   * without ending its spending authority would be a half-measure that reads
   * like a fix.
   */
  revokeAllSessions(account) {
    const s = this.db.prepare("DELETE FROM sessions WHERE account_id = ?").run(account.id);
    const g = this.db.prepare("UPDATE spend_grants SET revoked_at = ? WHERE account_id = ? AND revoked_at IS NULL")
      .run(now(), account.id);
    this.onEvent({ type: "account:signed-out-everywhere", account: account.id, sessions: s.changes, grants: g.changes });
    return { sessions: s.changes, grants: g.changes };
  }
  ...
  grantSpend(account, { address, maxMicro, expiresAt, ts, signature }) {
    ...
    /*
     * Single use (FIND-FIN-002).
     *
     * Authorizing a grant REPLACES the live one and starts spent_micro at
     * zero — which is right for a person deliberately re-authorizing, and was
     * a gift to anyone able to re-send the last one. The signed message is
     * only pinned to a timestamp, so for the whole freshness window the exact
     * same bytes were accepted again and again, and each acceptance handed
     * the cap back. Nothing distinguished the tenth replay from the first.
     *
     * The digest of the signed message is that authorization's identity, so
     * recording it makes it usable exactly once. A person re-authorizing
     * signs a new message with a new timestamp and is unaffected.
     */
    const digest = hash.toString("hex");
    try {
      this.db.prepare("INSERT INTO grant_nonces (digest, address, used_at) VALUES (?,?,?)")
        .run(digest, address, now());
    } catch (e) {
      // better-sqlite3 reports SQLITE_CONSTRAINT_PRIMARYKEY here; match the
      // family rather than the exact code, and fall back to the message so a
      // driver change cannot silently turn a replay into a 500.
      if (/SQLITE_CONSTRAINT/.test(String(e && e.code || "")) || /UNIQUE constraint/i.test(String(e && e.message || ""))) {
        throw fail("that grant authorization has already been used — approve it again from the app", 409);
      }
      throw e;
    }
    // Keep the table from growing without bound: nothing older than the
    // signature window can ever be presented again anyway.
    this.db.prepare("DELETE FROM grant_nonces WHERE used_at < ?").run(now() - 2 * LINK_SIG_WINDOW_MS);
  ...
  /**
   * Give back part of a hold (FIND-FIN-001). Reservations are taken at the
   * worst case a request could cost; this returns whatever it did not.
   * Floors at zero so a double refund can never mint spending room.
   */
  refundGrant(grantId, micro) {
    const amt = Math.max(0, Math.floor(Number(micro) || 0));
    if (!amt) return;
    this.db.prepare(
      "UPDATE spend_grants SET spent_micro = MAX(0, spent_micro - ?) WHERE id = ?"
    ).run(amt, String(grantId));
  }
  ...
  accountView(account) {
    return {
      ...
      /*
       * Grants ride along with the account view because every surface that
       * asks "who am I" also has to ask "may this browser spend, and how
       * much is left" — the web app renders a different application
       * depending on the answer. Splitting it into a second round-trip
       * would only guarantee the two answers are read a moment apart.
       */
...
function createAccounts(opts) {
  ...
  /**
   * Who is calling — without answering on their behalf. A page request wants
   * a door (a redirect to sign-in), not a JSON 401 the browser will render as
   * text, so it needs to ask the question without the gate writing a reply.
   * requireAccount is built ON this rather than beside it, so there stays
   * exactly one definition of what a valid session is.
   */
  const accountOf = (req) => svc.sessionAccount(tokenOf(req));
  ...
  router.post("/auth/email/verify", (req, res) => {
    try {
      ...
      /*
       * The cookie is the session; the body deliberately does NOT carry the
       * token. It used to, which put a 30-day credential somewhere page
       * JavaScript could read — so one XSS walked away with a durable
       * credential rather than being confined to the page it landed on. That
       * mattered less when a session only held an email address. Now a
       * session can authorise SPENDING through a grant, so it is a financial
       * credential and belongs in an httpOnly cookie only.
       *
       * The device-link poll still returns a token, and must: the desktop app
       * is not a browser and has no cookie jar. That is a different lane with
       * a different threat model, not an inconsistency.
       */
      setCookie(res, svc._issueSession(account.id, "web (email)"));
  ...
  router.post("/auth/passkey/login/verify", (req, res) => {
    try {
      ...
      setCookie(res, svc._issueSession(account.id, "web (passkey)"));
      res.json({ ok: true, account: svc.accountView(account) });
    } catch (e) { boom(res, e); }
  });
  ...
  router.get("/account/sessions", (req, res) => {
    const account = requireAccount(req, res);
    if (!account) return;
    res.json({ ok: true, sessions: svc.sessionList(account, tokenOf(req)) });
  });
  router.delete("/account/sessions/:id", (req, res) => {
    const account = requireAccount(req, res);
    if (!account) return;
    try {
      svc.revokeSessionById(account, req.params.id);
      res.json({ ok: true, sessions: svc.sessionList(account, tokenOf(req)) });
    } catch (e) { boom(res, e); }
  });
  router.post("/account/sessions/revoke-all", (req, res) => {
    const account = requireAccount(req, res);
    if (!account) return;
    const out = svc.revokeAllSessions(account);
    clearCookie(res);
   ...
   * non-GET.
   */
  return { router, service: svc, requireAccount, accountOf };
}
