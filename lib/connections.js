...
function mountConnections({ app, accounts, requireAdmin, stateDir, secret, siteOrigin, env, fetchImpl }) {
  ...
  require("./connection-events").mountEventRelay({ app, accounts, stateDir, secret, siteOrigin, transport, generation: () => settings.status().generation });
  ...
  app.post("/connections/api/:action", async (req, res) => {
    ...
    try {
      ...
      switch (action) {
        ...
        case "triggerTypes": result = await api.triggerTypes(input, signal); break;
        case "triggerUpsert": result = await api.triggerUpsert(input, userId, signal); break;
        case "triggerRemove": result = await api.triggerRemove(input, userId, signal); break;
