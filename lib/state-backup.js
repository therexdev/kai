...
function startBackups(dataDir, log = () => {}, beforeSnapshot = null) {
  const run = async () => {
    try {
      // In sqlite mode the DB is the authority and this bundler only reads
      // *.json — the hook re-derives current JSON views first so scheduled
      // snapshots never bundle stale ledgers (review finding).
      try { beforeSnapshot?.(); } catch { /* views are best-effort */ }
